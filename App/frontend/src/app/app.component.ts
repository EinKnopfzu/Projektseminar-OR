...
type HTTPGenerateResponse = { typ: string, prompt: string, response: string }[]
export interface ProductInformation { select_for_generate: boolean, display_name: string, order: number, message_array: MessageArray }
...
export class AppComponent {
  ...
  product_info_dict: { [product_id: string]: ProductInformation } = {
    "TitleAmazon": {display_name: "Titel", select_for_generate: false, order: 0, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) },
    "DescriptionLongShops": {display_name: "Produktbeschreibung", select_for_generate: false, order: 1, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) },
    "SalesArgument": {display_name: "Verkaufsargumente", select_for_generate: false, order: 2, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) },
    "AmazonBulletPoints": {display_name: "Bulletpoints", select_for_generate: false, order: 3, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) },
    "WorthKnowingShop": {display_name: "Worth Knowing", select_for_generate: false, order: 4, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) },
    "MetaKeywordShop": {display_name: "Meta Keywords", select_for_generate: false, order: 5, message_array: this.direct_chat_messages_array.map(object => ({ ...object })) }
  }
  ...
  generate_LLM_Response() {
    ...
    if(this.dummy_http_request) {
      ...
      this.http.post<HTTPGenerateResponse>('http://127.0.0.1:5000/generate', JSON.stringify(httpBody), httpOptions)
      .subscribe(response => {
